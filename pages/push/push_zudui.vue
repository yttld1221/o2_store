<template>
	<view class="content">
		...
		<uni-popup ref="huati_popup" type="bottom">
			<view class="huati_popup-box">
				...
				<view class="the-selected-labels">
					<view class="the-selected-labels-title">可添加多个话题(⌒∇⌒)哦~</view>
					<view class="tips">(提示：轻击可删除哦~)</view>
					<view class="the-selected-labels-label">
						<view @click="deletedOne(index)" class="label-one" v-for="(item,index) in theSelectedLabels">{{item}}</view>
					</view>
				</view>
				<view class="the-tip-labels">
					<view @click="addLabel(item)" class="label-one" v-for="(item,index) in theLabels">#{{item}}</view>
				</view>

				<!-- 评论输入框 -->
				<view class="comment-input" :style="'bottom:'+0+'px;'">
					<text>#</text>
					<uni-easyinput v-model="theHandAddLabel" @confirm="handAddLabel()" confirmType="next"
						placeholder="想要说点什么（￣ー+￣）..." :inputBorder="false" />
		...
		<view class="the-title">
			<view class="the-title-tip"><text class="red">*</text>{{type}}标题</view>
			<view class="the-title-input">
				<uni-easyinput :inputBorder="false" v-model="theData.title" placeholder="(*´∇｀*)快来取个标题吧~" />
			</view>
		</view>
		<!-- 内容 -->
		<view class="the-title">
			<view class="the-title-tip"><text class="red">*</text>内容</view>
			<view class="the-title-input-2">
				<uni-easyinput autoHeight type="textarea" :inputBorder="false" v-model="theData.content"
					placeholder="想要说点什么呢σ(ﾟｰ^*)~" />
			</view>
		</view>
		<!-- 图片 -->
		<view class="the-pic">
			<uni-file-picker @select="imageSelect" @delete="imageDelete" file-mediatype="image" v-model="imageValue" title="最多选择9张图片"></uni-file-picker>
		</view>
		<!-- 视频 -->
		<view class="the-pic" @click="$public.disabled_tip('上传视频')">
			<uni-file-picker disabled file-mediatype="video" v-model="videoValue" title="上传视频" limit="1"></uni-file-picker>
		</view>
		<!-- 话题 && 匿名发布按钮-->
		<view class="the-buttons">
			<view @click="openPopup_huati" :class="{'the-button':true,'label_selected':theSelectedLabels.length != 0}">#添加话题(
				{{theSelectedLabels.length}} )
			</view>
			<view @click="toAnonymous" :class="{'the-button':true,'the-button-1':theData.is_anonymous == 1}">
				<image v-if="theData.is_anonymous == 2" src="/static/3_niming_1.png" mode="widthFix"></image>
				<image v-if="theData.is_anonymous == 1" src="/static/3_niming_2.png" mode="widthFix"></image>
				<view :class="{'is_anonymous':theData.is_anonymous == 1}">匿名发布</view>
			</view>
		</view>
		<!-- 公开范围：帖子开放的区域 -->
		<view class="the-range" @click="toRange">
			<view class="the-range-title"><text class="white">*</text>发布范围：</view>
			<view class="the-range-selected">
				<view v-if="theSelectedranges.length == 0" class="the-range-selected-one">默认不限</view>
				<view v-if="theSelectedranges.length != 0" class="the-range-selected-one">当前已选（ {{theSelectedranges.length}} ）
				</view>
			</view>
		...
		<view v-if="type == '组队/搭子'">
			...
			<view class="the-hope">
				...
				<view class="the-hope-selected">
					<uni-easyinput @focus="easyinputFocus('number')" @blur="easyinputBlur('number')" placeholder="请输入组队/报名的上限数~"
						confirm-type="done" type="number" :inputBorder="false" v-model="theData.hope_num" />
				</view>
			</view>
			<!-- 性别 -->
			<view class="the-hope">
				<view class="the-hope-title"><text class="white">*</text>性别要求：</view>
				<view class="the-hope-selected">
			...
			<!-- 费用方式 -->
			<view class="the-hope">
				<view class="the-hope-title"><text class="white">*</text>费用方式：</view>
				<view class="the-hope-selected">
					<view @click="selectedSettlementOne(index)" class="the-range-selected-one" v-for="(item,index) in settlements">
						<uni-icons v-if="theSettlementIndex == index" type="circle-filled" size="25" color="#f89f12"></uni-icons>
						<uni-icons v-if="theSettlementIndex != index" type="circle"  size="25" color="#bbbbbb"></uni-icons>
						<view>{{item}}</view>
					</view>
				</view>
			</view>
			...
			<view class="the-hope">
				...
				<view class="the-hope-selected">
					<uni-datetime-picker :border="false" v-model="dateRange" type="daterange" @change="dateMaskClick"/>
				</view>
			</view>
			<!-- 活动地点 -->
			<view class="the-range no-bodrder" @click="toAddress()">
				<view class="the-range-title"><text class="red">*</text>活动地点：</view>
				<view class="the-range-selected">
					<view v-if="theAddress.title == ''" class="the-range-selected-one gray">（选择活动地点）</view>
					<view v-if="theAddress.title != ''" style="width: 500px;" class="the-range-selected-one"> {{theAddress.title.length<9?theAddress.title:(theAddress.title).substring(0,9)+'...'}}
					</view>
				</view>
				<view class="the-range-icon">
					<uni-icons type="right"></uni-icons>
		...
		<view v-if="type == '兼职'">
			<!-- 活动地点 -->
			<view class="the-range no-bodrder" @click="toAddress()">
				<view class="the-range-title"><text class="red">*</text>兼职城市：</view>
				<view class="the-range-selected">
					<view v-if="theAddress.title == ''" class="the-range-selected-one gray">（选择兼职所在城市）</view>
					<view v-if="theAddress.title != ''" style="width: 500px;" class="the-range-selected-one"> {{theAddress.title.length<9?theAddress.title:(theAddress.title).substring(0,9)+'...'}}
					</view>
				</view>
				<view class="the-range-icon">
			...
			<view class="the-hope">
				...
				<view class="the-hope-selected">
					<uni-easyinput placeholder="请输入金额 或 填写如：面议~"
						confirm-type="done" type="text" :inputBorder="false" v-model="theData.wages"/>
				</view>
			</view>
			<!-- 结算单位 -->
			<view class="the-hope">
				<view class="the-hope-title"><text class="white">*</text>结算单位：</view>
				<view class="the-hope-selected">
					<view @click="selectedSettlement_1sOne(index)" class="the-range-selected-one" v-for="(item,index) in settlement_1s">
						<uni-icons v-if="theSettlement_1sIndex == index" type="circle-filled" size="25" color="#f89f12"></uni-icons>
						<uni-icons v-if="theSettlement_1sIndex != index" type="circle"  size="25" color="#bbbbbb"></uni-icons>
						<view>{{item}}</view>
					</view>
				</view>
			</view>
			<!-- 结算周期 -->
			<view class="the-hope">
				<view class="the-hope-title"><text class="white">*</text>结算周期：</view>
				<view class="the-hope-selected">
					<view @click="selectedSettlement_2sOne(index)" class="the-range-selected-one" v-for="(item,index) in settlement_2s">
						<uni-icons v-if="theSettlement_2sIndex == index" type="circle-filled" size="25" color="#f89f12"></uni-icons>
						<uni-icons v-if="theSettlement_2sIndex != index" type="circle"  size="25" color="#bbbbbb"></uni-icons>
						<view>{{item}}</view>
					</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// -----------------------核心接口-----------------------
			// 发布
			toPush:function(state){ // '存草稿' '立即发布'
				let that = this;
				let theDataUrl = [];
				return new Promise(function(resolve, reject){
					if(that.pushChecking()){
						...
						if(that.type == '兼职'){ // 兼职的话，在这里给结算方式赋值一下
			...
			imageSelect:function(e){
				...
				(async function(){
					...
					if(theStoreTempImageUrl.length != 0){ // 证明有图片添加进去了
