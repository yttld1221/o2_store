<template>
	<view class="content">
		...
		<view class="the-line-1" :style="'margin-top:'+(statusBarHeight+navBarHeight)+'px;'">
			...
			<view @click="toPersonal(theLogonUser.level)" class="the-line-1-left" :style="'background: url('+theLogonUser.avatar_url+');'"></view>
			<!-- 昵称等 -->
			<view class="the-line-1-right">
				...
				<view class="the-line-1-right-top">
					...
					<view  @click="toPersonal(theLogonUser.level)" class="the-line-1-right-top-left">
						<view class="the-line-1-right-top-left-name">{{theLogonUser.nick_name}}</view>
						<view class="the-line-1-right-top-left-num">
							绑定手机号：{{theLogonUser.phone != '' && theLogonUser.phone != null?(theLogonUser.phone.substring(0,3)+'****'+theLogonUser.phone.substring(7,11)):'还没绑哦（￣ー+￣）'}}
						</view>
					</view>
					<!-- 二维码 -->
					<image @click="openPopup_erweima" class="erweima" src="/static/5_erweima.png" mode="widthFix"></image>
				</view>
				<!-- 学校 专业 -->
		...
		<view class="the-line-4">
			<view @click="line_4_itemsSelected(index,'状态切换')" class="the-line-4-item" v-for="(item,index) in line_4_items">
				<view :class="{'the-line-4-item-name':true,'the-line-4-item-selected':index == line_4_items_index}">{{item}}
				</view>
