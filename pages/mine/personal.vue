<template>
	<view class="content">
		...
		<view @click="toEditAvatar" class="line-name margin-bottom-avatar">
			...
			<view class="line-name-avatar" :style="'background: url('+theUser.avatar_url+');'"></view>
		</view>
		<!-- 我的昵称 -->
		<view class="the-school margin-bottom" @click="toEditOther('昵称',theUser.nick_name)">
			<view class="the-school-title">我的昵称：</view>
			<view class="the-school-selected">
				<view class="the-school-selected-one"> {{theUser.nick_name}}
				</view>
			</view>
		...
		<view class="line-name margin-bottom">
			...
			<view class="line-name-content phone">
				<uni-easyinput v-model="parameters.phone" placeholder="请输入手机号码" :inputBorder="false" />
			</view>
			<view class="line-name-other">
				<view class="code-button" @click="sendCode" v-if="storePhone != parameters.phone && storeCodeSecond == 0">发送验证码
				</view>
		...
		<view v-if="storePhone != parameters.phone" class="line-name margin-bottom">
			...
			<view class="line-name-content phone">
				<uni-easyinput v-model="parameters.check_code" placeholder="请输入验证码" :inputBorder="false" />
			</view>
			<view :class="{'code-button':true,'code-button-gray':parameters.phone == '' || parameters.check_code == ''}"
				@click="toLogonRegister()">确认并提交</view>
		</view>
		...
		<view class="line-name margin-bottom">
			...
			<view class="line-name-content">
				<uni-easyinput focus :disabled="theLevel != 0" v-model="parameters.name" placeholder="请输入真实姓名"
					:inputBorder="false" />
			</view>
		</view>
		...
		<view class="the-school margin-bottom">
			...
			<view class="the-school-selected">
				...
				<view v-if="parameters.school_id != 0" style="width: 500px;" class="the-school-selected-one gray">
					{{theschoolName.length<9?theschoolName:theschoolName.substring(0,9)+'...'}}
				</view>
			</view>
		</view>
		...
		<view class="the-school">
			...
			<view class="the-school-selected">
				<uni-datetime-picker :border="false" :disabled="theLevel != 0" v-model="parameters.grad_date" type="date"
...
<script>
	export default {
		...
		onShow() {
			// 专门处理从修改头像界面返回过来的情况
			if (this.$store.state.tempImageUrl.length != 0) { // 证明是通过修改头像界面跳过来的
		...
		methods: {
			...
			sendCode: function() {
				if (true) {
					...
					return new Promise(function(resolve, reject) {
						...
						// 判断如果是空字符串，不应该调用接口
						if (that.parameters.phone != '' && that.parameters.phone != null && that.parameters.phone.length ==
							11) {
							uni.request({
			...
			inspect: function() {
				// 由于这个接口是修改个人信息和注册统一的，所以这里都要传，这里把个人信息赋值一下
				this.parameters.school_ids = (this.theUser.school_ids == '' || this.theUser.school_ids == null ? '' : this.theUser
				.school_ids);
				...
				} else if (this.parameters.phone != this.theUser.phone && this.parameters.phone != '' && this.parameters
					.check_code == '') {
					uni.showToast({
						title: '请获取并正确输入验证码~',
						duration: 2500,
