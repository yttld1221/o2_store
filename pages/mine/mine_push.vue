<template>
	<view class="content">
		...
		<view class="posts-data" v-for="(item,index) in school_datas">
			<post-type-zudui @toDetail="toDetail" @toOn="toOn" :isMine="true" :postsDataOneIndex="1" :theData="item"></post-type-zudui>
		</view>
	...
	export default {
		...
		methods:{
			...
			toOn:async function(option){
				...
				if(option.is_on == 1){ // 表示当前是上线，点击的目的是为了下线
					await this.$store.dispatch('upDownMyMoments',{
						id:option.id,
						is_on:2,
						type:'发布管理'
					})
					if(this.$store.state.isOn_true == true){ // 调用成功
