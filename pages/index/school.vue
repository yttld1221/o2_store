		...
		<view class="top">
			...
			<view class="selected">
				...
				<view class="selected-content">
					<uni-icons type="map-pin-ellipse" size="17" color="#ff812f"></uni-icons>
					<view class="margin-left-5">当前：</view>
					<view class="margin-left-5">{{schoolNow != ''?schoolNow.substring(0,9):'(空)'}}{{schoolNow.length>9?'…':''}}
					</view>
				</view>
			</view>
			<!-- 50px -->
			<view class="the-selected">
				<view class="the-selected-title">
					<text>{{tempSelectedSchool.title != ''?tempSelectedSchool.title:'默认全部校区'}}</text>
				</view>
				<uni-icons v-if="tempSelectedSchool.title != ''" @click="cleanSchool()" type="clear" size="25" color="#727272"></uni-icons>
				<view @click="confirmSchool()" class="the-button">确认选择</view>
			</view>
		</view>
		<!-- 列表 -->
		<view class="address-box" :style="'margin-top:'+(56+25+15+50)+'px;'">
			<address-recursion @selectingAddress="selectingSchool" :level="1" :searchValue="searchValue"
				:theData="tempschoolData"></address-recursion>
		</view>
		<!-- 底部垫层 -->
