<template>
	<view class="content">
		...
		<!-- 背景 -->
		<view class="bg" :style="'height:'+(statusBarHeight+navBarHeight+125)+'px;'"></view>

		<!-- 导航切换栏 -->
		<view class="titles" :style="'top:'+statusBarHeight+'px;height:'+navBarHeight+'px;'">
			<view class="titles-box">
				<view @click="toTitleOne(index)" :class="{'title-selected':index == theTitleIndex,'title':true}"
					v-for="(item,index) in titleName">{{item}}</view>
			</view>
		</view>
		<!-- 定位选择 -->
		<view @click="toAddress()" class="location" :style="'top:'+statusBarHeight+'px;height:'+navBarHeight+'px;'">
			<uni-icons class="margin-left-10" type="map-pin-ellipse" size="20"></uni-icons>
			<text
				class="font-15-blod margin-left-5">{{theAddress.title != undefined?theAddress.title.length < 4?theAddress.title:(theAddress.title).substring(0,3)+'...':'选择地区'}}</text>
			...
			<view class="searchs-2">
				<view class="search-box-2">
					<view class="search-picker" @click="toSchool()">
						<text
							v-if="theSchool.title != ''">{{theSchool.title.length < 5?theSchool.title:(theSchool.title).substring(0,4)+'...'}}</text>
						<text v-if="theSchool.title == ''">全部校区</text>
						<uni-icons class="margin-left-5" type="down" size="12"></uni-icons>
					</view>
					<view class="search-input">
						<input type="text" v-model="searchInputText" :adjust-position="fasle" cursor-spacing="20"
							@focus="inputBindFocus" @blur="inputBindBlur" @confirm="searchPost" confirm-type="search"
							placeholder="请输入搜索内容" />
					</view>
					<uni-icons @click="emptyInput('searchInputText')" v-if="searchInputText != ''" type="clear" size="20"
						color="#bbbbbb"></uni-icons>
			...
			<view class="posts">
				<view class="posts-titles">
					<view class="flex-row" :style="'width:'+titles.length*20+ 'vw;'">
						<view :class="{'posts-titles-one':true,'posts-titles-one-choised':schoolOneTitleIndex == index}"
							@click="choiseOneTitle(index)" v-for="(item,index) in (theLevel == 2?titles:tempTitles)">
				...
				<!-- 筛选 -->
				<image @click="$public.disabled_tip('高级筛选')" class="image-width-20" src="/static/1_shaixuan.png"
					mode="widthFix"></image>
			</view>
		</view>
		<!-- 内容 -->
		<view>
			<view class="posts-data" :style="(index == 0?'margin-top:'+(statusBarHeight+navBarHeight+125)+'px;':'')"
				v-for="(item,index) in school_datas">
				<post-type-zudui @toDetail="toDetail" @topPerSonalhome="topPerSonalhome" @toThumb="toThumb"
					@actionMore="actionMore"
					@zuduiButtons="zuduiButtons"
					v-if="titles[schoolOneTitleIndex] == item.type || titles[schoolOneTitleIndex] == '综合' "
					:postsDataOneIndex="index" :theData="item"></post-type-zudui>
			</view>
		</view>
...
<script>
	export default {
		...
		onShow() {
			...
			if (that.theAddress.title == '' || that.theAddress.title == undefined) { // 表示当前地址为空，可能是用户第一次打开小程序，没有本地缓存数据
				uni.showModal({
					title: '温馨提示：',
