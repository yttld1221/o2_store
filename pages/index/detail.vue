	...
	<view class="content">
		...
		<view v-if="!['兼职', ''].includes(detailData.type)" :style="'padding-bottom:' + bottomHeight + 'px;'">
			...
			<view class="comment">
				...
				<view @longpress="onLongPress(item)" class="comment-one" :key="index"
					...
					.then((res) => {
						...
						if (
							...
							uni.setNavigationBarTitle({
								title: this.detailData.type == "分享/安利" ?
									"安利详情" : this.detailData.type == "兼职" ?
									"兼职详情" : "组队详情",
							});
							...
							success: (res) => {
								...
								if (res.data.code == 0) {
									...
									_that.theComments.unshift({
										...
										nick_name: _that.$store.state.theLogonUser
											.nick_name,
										avatar_url: _that.$store.state.theLogonUser
											.avatar_url,
									});
									// 重置
