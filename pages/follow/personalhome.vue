<template>
	<view class="content">
		...
		<view class="the-line-1">
			<!-- 头像 -->
			<view class="the-line-1-left" :style="'background: url('+thePersonalInfo.avatar_url+');'"></view>
			<!-- 昵称等 -->
			<view class="the-line-1-right">
				...
				<view class="the-line-1-right-top">
					...
					<view class="the-line-1-right-top-left">
						<view class="the-line-1-right-top-left-name">{{thePersonalInfo.nick_name}}</view>
						<view class="the-line-1-right-top-left-num">最近登录：{{$public.showDateTime(thePersonalInfo.last_login_at)}}</view>
					</view>
					<!-- 分享 -->
					<image class="share" src="/static/2_share@3x.png" mode="widthFix"></image>
				</view>
				<!-- 学校 专业 -->
				<view class="the-line-1-right-bottom">
					<view class="the-line-1-right-bottom-school">
						...
						<view>{{thePersonalInfo.school_name != ''?thePersonalInfo.school_name:'学校不详，遇强则"降"▼ω▼'}}</view>
					</view>
		...
		<view :class="{'the-line-2':true,'gray':thePersonalInfo.intro == ''}">
			{{thePersonalInfo.intro != ''?thePersonalInfo.intro:'这个家伙很懒(^■^*)，没有写个人介绍哦~'}}
		</view>
		<!-- 第三行：关注和 粉丝数-->
		<view class="the-line-3">
			<view class="the-line-3-nums">
				<view class="the-line-3-nums-regard_num"><text>{{thePersonalInfo.regard_num}}</text> 关注</view>
				<view class="the-line-3-nums-regard_num"><text>{{thePersonalInfo.fans_num}}</text> 粉丝</view>
			</view>
			<view v-if="isMe()" class="the-line-3-button">
				<view @click="toRegard('关注TA')" v-if="thePersonalInfo.is_regard == 2" class="button">关注TA</view>
				<view @click="toRegard('取消关注')" v-if="thePersonalInfo.is_regard == 1" class="bg-gray">取消关注</view>
			</view>
		</view>
		<!-- 第四行：列表按钮 -->
		<view class="the-line-4">
			<view @click="line_4_itemsSelected(index)" class="the-line-4-item" v-for="(item,index) in line_4_items">
				<view :class="{'the-line-4-item-name':true,'the-line-4-item-selected':index == line_4_items_index}">{{item}}</view>
		...
		<view  class="the-line-5">
			<view class="posts-data" v-for="(item,index) in school_datas">
				<post-type-zudui @toDetail="toDetail" @toThumb="toThumb" @zuduiButtons="zuduiButtons"
				:isPersonalHome="true" :postsDataOneIndex="-1" :theData="item"></post-type-zudui>
			</view>
		</view>
		<!-- 底部垫层 -->
...
<style>
	.bg {
		...
		width: 100vw;
		height: 110vw;
		background-image: linear-gradient(to bottom right, #ffffff, #ffffff, #f4fcf5, #f4fcf5);
		z-index: -1;
	}
	.margin-left-10 {
		margin-left: 10px;
