<template>
	<view class="tabbar">
		...
		<view :class="{'tabbar-item':true,'tabbar-item-push':index == 2}" v-for="(item,index) in list" :key="index"
			@click="changeTab(index)">
			<view>
				<image :animation="index == 3?animationData:''" :class="{'img':true,'push':index == 2}" mode="widthFix" :src="item.selectedIconPath"
					v-if="current == index"></image>
				<image :animation="index == 3?animationData:''" :class="{'img':true,'push':index == 2}" mode="widthFix" :src="item.iconPath" v-else></image>
				<view class="text active" v-if="current == index">{{item.text}}</view>
				<view class="text" v-else>{{item.text}}<text v-if="index == 3 && isRedTip" class="red-tip"></text></view>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			changeTab(e) {
				if (e == 2) {
					if (this.$store.state.isPage_2 == true) { // 表示已经在发布页，证明是第二次点击
						uni.switchTab({
