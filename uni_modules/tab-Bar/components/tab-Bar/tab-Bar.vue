		...
		<view class="tabbar-box">
			<view :class="{ 'tabbar-item': true, 'tabbar-item-push': index == 2 }" v-for="(item, index) in list"
				...
				<view class="item-box" :class="{ 'img-box': index == 2 }">
					...
					<image :animation="index == 3 ? animationData : ''" :class="{ img: index != 2, push: index == 2 }"
						:mode="index != 2?'widthFix':''" :src="item.iconPath" v-else></image>
					<image class="push-hidden" :class="{ img: true }" mode="widthFix"
						src="@/static/2_tabbar_follow@3x.png" v-if="index == 2"></image>
					<view class="text active" v-if="momentIndex == index">{{
            item.text
          }}</view>
					<view class="text" v-else>{{ item.text
            }}<text v-if="index == 3 && isRedTip" class="red-tip"></text></view>
				</view>
			</view>
		</view>
		<u-popup @close="closePop()" :duration="50" :safeAreaInsetBottom="false" :round="10" :show="show" mode="bottom">
			<view class="push-pop">
				<view class="pop-title">选择分类</view>
				<view class="push-container">
					<view @click="toPush(item)" class="push-item" v-for="(item, index) in pushListTop" :key="index">
						<image :src="item.url"></image>
						{{ item.label }}
					</view>
				</view>
				<view class="push-container">
					<view @click="toPush(item)" class="push-item" v-for="(item, index) in pushListBottom" :key="index">
						<image :src="item.url"></image>
						{{ item.label }}
					</view>
				</view>
			...
			changeTab(e) {
				if (e == 2) {
					if ([1, 2].includes(this.$store.state.theLogonUser.level)) {
						...
						uni.navigateTo({
							url: "/page_product/pages/info/register?level=" +
...
<style lang="scss">
	...
	.tabbar-box {
		...
		width: 100%;
		display: flex;
		height: calc(202rpx + calc(env(safe-area-inset-bottom) / 2));
		align-items: flex-end;
		justify-content: space-around;
		// background-color: #fff;
		background: url('/static/icon-tabbar.png');
	...
	.push-pop {
		...
		padding-bottom: calc(125rpx + calc(env(safe-area-inset-bottom) / 2));
