		...
		<view class="jz-box" v-if="theData.type == '兼职'">
			...
			<view class="jz-top">
				...
				<view class="type-line-1">
					...
					<view class="type-line-1-amount">{{ theData.wages
              ...
              theData.settlement | getSettlement
				...
				<view class="jz-labels" v-if="label.length">
					<view class="line-2-one jz-labels-item" :key="index" v-for="(item, index) in label">{{ item }}
					</view>
				</view>
				<!-- 地址和结算方式 -->
              ...
              theData.area_name
					...
					<view v-if="theData.is_on == 1 && showPhone" class="phone-btn" @click.stop="topPerSonalhome">立即联系
					</view>
				</view>
			</view>
		</view>
		<view class="compoent-al" v-if="theData.type == '分享/安利' && compoentType == 'list'">
			...
			<view class="al-content">
				...
				<view class="al-bottom flex-algin">
					...
					<view class="flex-algin" @click.stop="toThumb">
						<uni-icons type="hand-up" :color="theData.is_thumb == 2 ? '#868686' : '#ff6155'"
							size="20"></uni-icons>
						<view :style="{
                color: theData.is_thumb == 2 ? '#868686' : '#ff6155',
              }" class="al-bottom-num">{{ theData.thumb_num }}</view>
					</view>
				</view>
			</view>
		</view>
		<view class="other-box" v-if="
        !['兼职', '分享/安利'].includes(theData.type) ||
        (theData.type == '分享/安利' && compoentType != 'list')
      ">
			<!-- 头像等 -->
			<view @click.stop="topPerSonalhome" class="avatar" :style="'background: url(' + theData.avatar_url + ');'">
			</view>
			<view class="post-right">
				...
				<view class="line-2" v-if="label.length">
					<view class="line-2-one" :key="index" v-for="(item, index) in label">#{{ item }}</view>
				</view>
				<!-- 标题 -->
				<view class="line-3">{{ theData.title }}</view>
				<view v-if="getPictures.length" class="line-4" :class="{ 'pad-r-pic': getPictures.length == 4 }">
					<image mode="aspectFill" :src="
              item +
              '?x-oss-process=image/resize,m_lfit,h_800,w_800/quality,q_100'
            " @click.stop="$public.previewImage(pictures, index)" class="picture" :class="{
              'mar-r-b': getPictures.length != 4 && [2, 5, 8].includes(index),
            }" :key="index" v-for="(item, index) in getPictures" />
				</view>
				<!-- 描述内容，只有详情页面才展示 -->
              ...
              theData.entry_num + " / " + theData.hope_num
					...
					<view class="the-progress">
						<progress :percent="(theData.entry_num / theData.hope_num) * 100" border-radius="100"
							stroke-width="10" activeColor="#F89F12" backgroundColor="#F6F6F6"></progress>
					</view>
					...
					<view class="progress-button" v-if="theData.is_on == 1">
						<button @click.stop="zuduiButtons(0)" open-type="share" class="progress-button-one-1">
							<uni-icons type="redo-filled" color="#ffffff" size="25"></uni-icons>
							<view class="progress-button-name">邀请好友</view>
						</button>
						<view @click.stop="zuduiButtons(1)" :class="{
                'progress-button-one-2': true,
                'bg-gray': theData.is_entry == 1,
              }">
							<uni-icons type="staff-filled" color="#ffffff" size="25"></uni-icons>
				...
				<view class="line-6">
					<view class="the-school">
						<image class="school-icon" src="/static/1_school_icon@3x.png"></image>
						<view class="school-name">{{ theData.school_name }}</view>
					</view>
					<view class="the-thumb-comment-box" v-if="!isPersonalHome && theData.is_on == 1 && !isMine">
						<view @click.stop="toThumb" class="flex-row">
							<uni-icons type="hand-up" :color="theData.is_thumb == 2 ? '#000000' : '#ff6155'"
								size="20"></uni-icons>
				...
				<view v-if="isDetail == true && theData.type == '组队/搭子'" class="line-7">
					...
					<view class="line-7-member-avatars">
						<image :key="index" class="line-7-member-avatars-image" :src="item.avatar_url"
							v-for="(item, index) in theData.members"></image>
					</view>
				</view>
				<view v-if="!isDetail" class="read-more-box">
					<view class="more-list-box">
						<view v-for="(item,index) in plList" :key='index' class="more-list-item">
							<view class="avatar-box">
								<view class="avatar-left" @click.stop="toPlPerSonalhome(item)">
									<view class="avatar" :style="'background: url(' + item.avatar_url + ');'">
									</view>
									<view class="nick-name">{{item.nick_name}}</view>
									<view class="create-time">{{getTime(item.created_at)}}</view>
								</view>
								<view @click.stop="toThumb" class="flex-row">
							...
							<view class="msg-box">{{item.msg}}</view>
							<readMore childLen='10' :parentId="item.id"></readMore>
						</view>
					</view>
				</view>
			</view>
		...
		data() {
			return {
				plList: [{
					...
					nick_name: "姓名1",
					created_at: "2024-07-28 15:34:16",
					msg: "有事需要代课111"
				}],
				// 发帖时间
				released_at: "",
