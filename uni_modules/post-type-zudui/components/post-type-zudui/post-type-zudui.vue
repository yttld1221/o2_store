<template>
	<!-- 组队类型 -->
	<view @click="toDetail" :class="{'content':true,'posts-data-one':postsDataOneIndex == 0}">
		<view v-if="theData.type != '兼职' && type != '分享/安利'">
			<!-- 头像等 -->
			<view class="line-1">
				<view class="line-1-left">
					<view @click.stop="topPerSonalhome" class="avatar" :style="'background: url('+theData.avatar_url+');'"></view>
			...
			<!-- 话题 -->
			<view class="line-2">
				<view class="line-2-one" v-for="(item,index) in label">#{{item != ''?item:'(☆▽☆)#'}}</view>
			</view>
			<!-- 标题 -->
			...
			<!-- 图片 -->
			<view class="line-4">
				<view @click.stop="$public.previewImage(pictures)" v-if="item != ''" class="picture" :style="'background: url('+item+'?x-oss-process=image/resize,m_lfit,h_160,w_160'+');'" v-for="(item,index) in getPictures"></view>
			</view>
			<text @click.stop="$public.previewImage(pictures)" v-if="pictures.length" class="pic-tip">共 {{pictures.length}} 张，点击图片查看全部</text>
			<!-- 描述内容，只有详情页面才展示 -->
				...
				<view class="progress-tip">
					<view class="progress-tip-title">
						...
						<view class="sex-icon-type">
							<text>(</text>
							<image class="sex-icon" src="/static/1_sex.png" mode="widthFix"></image>
							<view class="sex-type">{{theData.sex_type}}</view>
							<image class="sex-icon" src="/static/1_feiyong.png" mode="widthFix"></image>
							<view class="sex-type">{{theData.free_type}}</view>
							<text>)</text>
						</view>
					</view>
					<view  v-if="isPersonalHome == false" class="progress-tip-num">{{theData.entry_num+' / '+theData.hope_num}}</view>
				</view>
				<view v-if="isPersonalHome == false" class="the-progress">
					<progress :percent="theData.entry_num/theData.hope_num*100" border-radius="100" stroke-width="10" activeColor="#f89f12" backgroundColor="#f6f6f6"></progress>
				</view>
				<view v-if="isPersonalHome == false && isMine == false" class="progress-button">
					...
					<view @click.stop="zuduiButtons(1)" :class="{'progress-button-one-2':true,'bg-gray':theData.is_entry == 1}">
						<uni-icons type="auth-filled" color="#ffffff" size="25"></uni-icons>
						<view class="progress-button-name">{{theData.is_entry == 2?'加入组队':'退出组队'}}</view>
					</view>
				</view>
			</view>
			<!-- 学校、点赞数等 -->
			<view class="line-6">
				<view class="the-school">
					<image class="school-icon" src="/static/1_school_icon@3x.png" mode="widthFix"></image>
					<view class="school-name">{{theData.area_name}}{{(theData.area_name!=''&&theData.school_name)!=''?'·':''}}{{theData.school_name}}</view>
				</view>
				<view class="the-thumb-comment-box">
					<view v-if="isPersonalHome == false" @click.stop="toThumb" class="flex-row">
						<uni-icons type="hand-up" :color="theData.is_thumb == 2?'#000000':'#ff6155'" size="20"></uni-icons>
						<view class="the-thumb-num" :style="theData.is_thumb == 2?'#000000':'color: #ff6155;'">{{theData.thumb_num}}</view>
					</view>
					...
					<view v-if="isPersonalHome != false" class="flex-row the-comment">
						<!-- <uni-icons type="chatbubble" size="20"></uni-icons> -->
						<view class="the-thumb-num"><text style="color: #ff812f;margin-right: 5px;">@{{theData.type}}@</text><text style="color: #bbbbbb;">详情 >> </text></view>
					</view>
				</view>
			</view>
			<!-- 组队/搭子 有效，仅在详情页显示，已经报名的人 -->
			<view v-if="isDetail == true && theData.type == '组队/搭子'" class="line-7">
				<view class="line-7-line-space"></view>
				<view class="line-7-line-title">组队成员（{{theData.members.length}}）</view>
				<view class="line-7-member-avatars">
					<image class="line-7-member-avatars-image" :src="item.avatar_url" mode="widthFix" v-for="(item,index) in theData.members"></image>
				</view>
			</view>
		</view>
		<view v-if="theData.type == '兼职'">
			...
			<view class="type-line-2">
				<view class="type-line-2-address">
					<uni-icons type="map-pin-ellipse" size="20" color="#ff812f"></uni-icons>
		...
		<view v-if="isMine" class="is-on-line">
			<view class="is-on-line-status">当前状态：{{theData.is_on == 1?'已上线':'草稿箱'}}</view>
			<view @click.stop="" :class="{'progress-button-one-2':true,'bg-gray':theData.is_on == 1}">
				<uni-icons type="paperplane-filled" color="#ffffff" size="25"></uni-icons>
				<view @click="toOn()" class="progress-button-name">{{theData.is_on != 1?'上线发布':'下线撤回'}}</view>
			</view>
		</view>
		<!-- 间隔 -->
	...
	export default {
		...
		mounted() {
			...
			setTimeout(function(){
				...
				if(that.theData.type == '组队/搭子'){ // 只有组队/搭子，才需要处理这个
		...
		computed:{
			getPictures:function(){
				...
				if(this.pictures.length > 4){
					for(let i=0;i<this.pictures[i].length;i++){
		...
		methods:{
			...
			topPerSonalhome:function(){
				this.$emit('topPerSonalhome',{id:this.theData.create_id,is_anonymous:this.theData.is_anonymous})
			},
			toThumb:function(){
				this.$emit('toThumb',{
					id:this.theData.id,
			...
			// 邀请好友 加入/退出组队
			zuduiButtons:function(index){ // 传0表示是邀请，传1是组队按钮
...
<style>
	...
	.posts-data-one {
		background-image: linear-gradient(to bottom, #fff0e8, #fffdfd, #ffffff) !important;
	}
