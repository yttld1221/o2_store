<template>
  <!-- 组队类型 -->
  <view @click="toDetail" :class="{ content: true }">
    <view v-if="theData.type != '兼职' && type != '分享/安利'">
      <!-- 头像等 -->
      <view class="line-1" :class="{ 'mar-t-0': postsDataOneIndex == 0 }">
...
.nick-more {
  width: calc(100% - 83.1rpx);
  margin: 13rpx 0 0 15rpx;
  display: flex;
  align-items: center;
