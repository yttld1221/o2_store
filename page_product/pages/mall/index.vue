		...
		<view class="mall-top">
          ...
          userInfo.intro ? userInfo.intro : "这家伙很神秘，没有写个人简介。"
				...
				<view class="info-school flex-algin">
					...
					<view v-if="theLogonUser.specialty" class="info-school-major info-school-box flex-algin">
						{{ theLogonUser.specialty }}</view>
				</view>
				<view class="other-info flex-algin">
					<view class="other-info-left flex-algin">
						<view class="flex-algin"><text>{{ userInfo.regard_num }}</text>关注</view>
					...
					<view class="other-info-right" :style="{ visibility: userId != theLogonUser.id ? '' : 'hidden' }">
						...
						<view @click="toChat()" class="sx-btn">私信</view>
					</view>
				</view>
			</view>
			<view class="tab-container flex-algin">
				<view class="tab-box flex-algin">
					<view @click="changeTab(index)" class="tab-item" :class="{ choose: current == index }"
						v-for="(item, index) in list" :key="index">
						<view class="heng" v-if="current == index"></view>
						<text>{{ item.label }}</text>
					</view>
				</view>
				<u-icon @click="toSearch()" name="search" color="#333333" size="23"></u-icon>
			</view>
		</view>
		<view :style="'margin-top:' + contentHeight + 'px;'" class="list-box" :class="{ 'shop-list': current == 0 }">
			<view class="shop-list-box" v-if="current == 0">
				<view @click="goDetail(item)" class="list-item" v-for="(item, index) in shopList" :key="index">
			...
			<view v-else>
				<view class="posts-data" v-for="(item, index) in shopList">
					<post-type-zudui :showPhone="false" @toJzDetail="toJzDetail" @toDetail="toDetail" @toThumb="toThumb"
						@actionMore="actionMore" @zuduiButtons="zuduiButtons" :postsDataOneIndex="-1"
...
<script>
	export default {
		...
		methods: {
			...
			toFans() {
				uni.navigateTo({
					url: "/page_product/pages/fans/index?userId=" +
						this.userId +
						"&name=" +
