<template>
  <view class="content">
      ...
      <view class="the-selected">
        <view class="the-selected-labels-label">
          <view
            ...
            class="label-one"
            v-for="(item, index) in theSelectedranges"
            >{{ item.title }}</view
          >
        </view>
      </view>
    ...
    <view
      class="address-box"
      :style="'margin-top:' + (56 + 25 + 15 + 150) + 'px;'"
    >
      <address-recursion
...
export default {
  ...
  onLoad(options) {
    ...
    // --------------------------------------------------------------调用初始数据--------------------------------------------------------------
    // --------------------------------------------------------------调用初始数据--------------------------------------------------------------
    this.theSelectedranges = JSON.parse(decodeURIComponent(options.list));
    this.getAreaTree();
  },
  onShow() {
    // 同步一下
  ...
  methods: {
    ...
    selectingAddress: function (arr) {
      if (arr[1] == "省") {
        ...
        // });
        // uni.$emit("getRange", this.theSelectedranges);
