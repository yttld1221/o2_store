	...
	<view class="push">
		<view class="push-form">
			...
			<view class="upload-box" v-if="type != '兼职'">
				<view @click="prewFile(item, index)" class="filelist-item" v-for="(item, index) in fileList"
					:key="index">
				...
				<view class="upload-btn" v-if="fileList.length < 9" @click="chooseMedia()">
					<u-icon name="camera" color="#FFFFFF" size="50"></u-icon>
				</view>
			</view>
			<view class="the-buttons" v-if="type != '分享/安利'">
				<view @click="openPopup_huati" :class="{
            'the-button': true,
            label_selected: theSelectedLabels.length,
          }">#添加话题
				</view>
				<view v-if="!['分享/安利', '兼职', '组队/搭子'].includes(type)" @click="toAnonymous" :class="{
            'the-button': true,
          ...
          type == "分享/安利" ? "选择范围" : "公开范围"
			...
			<view v-else>
				...
				<view class="choose-item form-content">
					...
					<u-radio-group v-model="theData.free_type" placement="row">
						...
						<u-radio activeColor="#f89f12" name="其他" label="其他"></u-radio>
					</u-radio-group>
				</view>
			</view>
			<view v-if="['兼职'].includes(type)">
				...
				<view v-if="theData.wages != '面议'">
					<view class="choose-item">
						...
						<u-radio-group v-model="theData.jsdw" placement="row">
							<u-radio activeColor="#f89f12" name="元/小时" label="元/小时"></u-radio>
							<u-radio activeColor="#f89f12" name="元/天" label="元/天"></u-radio>
							<u-radio activeColor="#f89f12" name="元/月" label="元/月"></u-radio>
						</u-radio-group>
					</view>
					<view class="choose-item">
				...
				<view class="choose-item" @click="toAddress()">
					...
					<view class="flex-algin">
						<view class="right-text" :class="{ 'choose-text': !theData.area_name }">{{
			...
			<view class="choose-item" v-if="['分享/安利'].includes(type)" @click="chooseProduct()">
				<view class="left-label">添加商品链接</view>
				<view class="flex-algin right-product">
					<view class="right-text product-title" :class="{ 'choose-text': !taskTitle }">
						{{ taskTitle ? taskTitle : "（请选择关联商品）" }}
					</view>
					<u-icon slot="right" name="arrow-right"></u-icon>
				</view>
			</view>
		...
		<view class="fix-bottom-box">
			<view class="fix-bottom">
				<view class="cgx-box" @click="save(2)" v-if="!['组队/搭子', '分享/安利'].includes(type)">
		...
		<uni-popup ref="huati_popup" type="bottom">
			<view class="huati_popup-box">
				...
				<view class="the-tip-labels">
					<view @click="addLabel(item)" class="label-one" v-for="(item, index) in theLabels">#{{ item }}
					</view>
				</view>
				...
				<view class="comment-input" :style="'bottom:' + 0 + 'px;'">
					...
					<uni-easyinput v-model="theHandAddLabel" @confirm="handAddLabel()" confirmType="next"
						placeholder="想要说点什么..." :inputBorder="false" />
...
<script>
	import {
		addressList
	} from "../../components/piaoyi-cityPicker/cityData";
	export default {
		components: {},
		data() {
			return {
		...
		onLoad(options) {
			...
			if (options.type) {
				...
				uni.setNavigationBarTitle({
					title: this.type == "分享/安利" ?
						this.titleObj[options.type] : `${options.id ? "编辑" : "发布"}${this.titleObj[options.type]}`,
				});
			}
			if (options.id) {
				this.id = options.id;
			...
			getDetail(id) {
				this.API.home
					...
					.then((res) => {
						...
						this.dateRange = res.data.start_at ? [res.data.start_at, res.data.end_at] : [];
