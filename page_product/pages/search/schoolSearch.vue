	...
	<view class="search-result">
		<view class="top-fixd">
			<view class="top-search">
				<u-search placeholder="请输入搜索关键词" v-model="searchText" :showAction="true" actionText="取消"
					:clearabled="true" @change="changeText" @search="searchReasult" @custom="cancel"></u-search>
			</view>
			<view class="type-container">
				<scroll-view class="scroll-view" scroll-x="true" scroll-with-animation="true">
					<view class="type-item" @click="changeType(item, index)" v-for="(item, index) in titles"
						:key="index">
		...
		<view :class="{ 'al-box': titles[chooseIndex] == '分享/安利' }" class="list-container"
			:style="'margin-top:' + contentHeight + 'px;'">
			<view :class="titles[chooseIndex] == '分享/安利' ? 'al-item' : 'posts-data'" :key="index"
				v-for="(item, index) in list">
				<post-type-zudui compoentType="list" @toDetail="toDetail" @topPerSonalhome="topPerSonalhome"
					@toThumb="toThumb" @actionMore="actionMore" @zuduiButtons="zuduiButtons" :postsDataOneIndex="-1"
		...
		methods: {
			...
			searchReasult() {
				if (this.searchText) {
					...
					uni.setStorageSync('searchSchoolRecord', JSON.stringify(this.searchRecord));
				}
				this.theGetMomentsListPage = 1;
				this.list = [];
				this.getList();
